...
//conseguir el id de los chat users
...
function followUser($user_id)
{
    ...
    createNotification($cu['id'], $user_id, "ha comenzado a seguirte!");
    return mysqli_query($db, $query);
}
...
function blockUser($blocked_user_id)
{
    ...
    createNotification($cu['id'], $blocked_user_id, "te ha bloqueado");
...
function unblockUser($user_id)
{
    ...
    createNotification($current_user, $user_id, "te ha desbloqueado!");
    return mysqli_query($db, $query);
}

//funcion checkLikeStatus
...
function like($post_id)
{
    ...
    if ($poster_id != $current_user) {
        createNotification($current_user, $poster_id, "le ha dado me gusta a tu publicación!", $post_id);
    }
...
function addComment($post_id, $comment)
{
    ...
    if ($poster_id != $current_user) {
        createNotification($current_user, $poster_id, "ha comentado en tu publicación", $post_id);
    }
...
function unlike($post_id)
{
    ...
    $poster_id = getPosterId($post_id);
    if ($poster_id != $current_user) {
        //createNotification($current_user, $poster_id, "", $post_id);
    }
...
function unfollowUser($user_id)
{
    ...
    //createNotification($current_user, $user_id, "Te ha dejado de seguir"
    return mysqli_query($db, $query);
}
...
function validateSignupForm($form_data)
{
    ...
    if (!$form_data['password']) {
        $response['msg'] = "Falta la contraseña";
        $response['status'] = false;
        $response['field'] = 'password';
    }

    if (!$form_data['username']) {
        $response['msg'] = "Falta el nombre de usuario";
        $response['status'] = false;
        $response['field'] = 'username';
    }

    if (!$form_data['email']) {
        $response['msg'] = "Falta el correo";
        $response['status'] = false;
        $response['field'] = 'email';
    }

    if (!$form_data['last_name']) {
        $response['msg'] = "Falta el apellido";
        $response['status'] = false;
        $response['field'] = 'last_name';
    }
    if (!$form_data['first_name']) {
        $response['msg'] = "Falta el nombre";
        $response['status'] = false;
    ...
    if (isUsernameRegistered($form_data['username'])) {
        $response['msg'] = "Nombre de usuario ya registrado";
        $response['status'] = false;
        $response['field'] = 'username';
    }
...
function validateLoginForm($form_data)
{
    ...
    if (!$form_data['username_email']) {
        $response['msg'] = "Falta el nombre de usuario/correo";
        $response['status'] = false;
    ...
    if (!$blank && !checkUser($form_data)['status']) {
        $response['msg'] = "No se ha encontrado su cuenta";
        $response['status'] = false;
...
//comprobar si se sigue a un usuario o no
...
//comprobar si te sigue un usuario o no
...
//conseguir usuarios para sugeridos 
...
//conseguir el id del que publica el post
...
//conseguir usuarios por nombre de usuario
...
//conseguir posts de manera dinámica
...
//para validar el formulario de reset
function validateUpdateForm($form_data, $image_data)
{
    ...
    if (isUsernameRegisteredByOther($form_data['username'])) {
        $response['msg'] = $form_data['username'] . " ya está registrado.";
        $response['status'] = false;
        $response['field'] = 'username';
    }
    ...
    if ($image_data['name']) {
        ...
        if ($type != 'jpg' && $type != 'jpeg' && $type != 'png') {
            $response['msg'] = "Solo se admiten imágenes en formato PNG, JPG y JPEG";
            $response['status'] = false;
            $response['field'] = 'profile_pic';
        }

        if ($size > 10000) {
            $response['msg'] = "Sube imágenes de menos de 10mb";
            $response['status'] = false;
            $response['field'] = 'profile_pic';
        }
    }
...
function validatePostImage($image_data)
{
    ...
    if (!$image_data['name']) {
        $response['msg'] = "No hay una imagen seleccionada";
        $response['status'] = false;
