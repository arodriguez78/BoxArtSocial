...
<div class="container col-md-10 col-sm-12 col-lg-9 rounded-0 d-flex justify-content-between bg-white">
    <div class="col-md-8 col-sm-12" style="max-width:93vw">
        ...
        showError('post_img');
        if (count($posts) < 1) {
            echo "<p style='width:40vw' class='p-2 bg-white border rounded text-center my-3 col-12'>Sigue a alguien o añade alguna publicación</p>";
        }
        ...
        ?>
            <div class="card mt-4">
                        ...
                        ?>
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="option<?= $post['id'] ?>">
                                    <li><a class="dropdown-item" href="assets/php/actions.php?deletepost=<?= $post['id'] ?>"><i class="bi bi-trash-fill"></i> Borrar publicación</a></li>
                                </ul>
                            </div>
                        <?php
                        }
                ...
                <h4 style="font-size: x-larger" class="p-2 border-bottom d-flex">
                    ...
                    &nbsp;&nbsp;<i class="bi bi-chat-left d-flex align-items-center" style="cursor: pointer;" data-bs-toogle="modal" data-bs-target="#postview<?= $post['id'] ?>"><span class="p-1 mx-2 text-small" style="font-size:small" data-bs-toggle="modal" data-bs-target="#postview<?= $post['id'] ?>"><?= count($comments) ?> comentarios</span></i>
                ...
                <div class="input-group p-2 <?= $post['post_text'] ? 'border-top' : '' ?>">
                    ...
                    <input type="text" class="form-control rounded-0 border-0 comment-input" placeholder="Di algo..." aria-label="Recipient's username" aria-describedby="button-addon2">
                    <button class="btn btn-outline-primary rounded-0 border-0 add-comment" data-page='wall' data-cs="comment-section<?= $post['id'] ?>" data-post-id="<?= $post['id'] ?>" type="button" id="button-addon2">Publicar</button>
                </div>
                        ...
                        <div class="modal-body d-md-flex p-0">
                            ...
                            <div class="col-md-4 col-sm-12 d-flex flex-column">
                                ...
                                <div class="flex-fill align-self-stretch overflow-auto" id="comment-section<?= $post['id'] ?>" style="height: 100px;">
                                    ...
                                    if (count($comments) < 1) {
                                    ?>
                                        <p class="p-3 text-center my-2 nce">sin comentarios</p>
                                    <?php
                                    }
                                ...
                                <div class="input-group p-2 border-top">
                                    <input type="text" class="form-control rounded-0 border-0 comment-input" placeholder="Di algo..." aria-label="Recipient's username" aria-describedby="button-addon2">
                                    <button class="btn btn-outline-primary rounded-0 border-0 add-comment" data-cs="comment-section<?= $post['id'] ?>" data-post-id="<?= $post['id'] ?>" type="button" id="button-addon2">Publicar</button>
                                </div>
                            </div>
    ...
    <div class="col-lg-4 col-sm-0 overflow-hidden mt-4 p-sm-0 p-md-3">
        ...
        <div>
            <h6 class="text-muted p-2">Quizás te interese seguir a...</h6>
            <?php
